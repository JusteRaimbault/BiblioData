		...
		public static void main(String[] args) {
            ...
            if (args.length == 0) {
                System.out.println("Usage : --citation\n" +
                        ...
                        "| --mongo-ids $DATABASE $IDS_FILE [$CONSOLIDATIONDATABASE]\n" +
                    ...
                    if (args.length == 1) {
                        ...
                        if (args.length >= 5) {
                            if (args[4].equals("true")) {
                                consolidationDatabase = Context.getCentralDatabase();
                            } else {
                                consolidationDatabase = args[4];
                            }
                        }
                        if (args.length == 6) {
                ...
                if (action.equals("--mongo-ids")) {
                    if (args.length == 1) {
                        System.out.println("Constructing citation network from mongo with specified ids. Usage : --citation --mongo-ids \n" +
                                "  $DATABASE: name of the database \n" +
                                "  $IDS_FILE: csv file (ids first column) with ref to collect\n" +
                                "  [$CONSOLIDATION] (optional) : if true, default central db, otherwise name of consolidation database"
                        );
                    } else {
                        Log.stdout("Citation network from mongo with specified refs");
                        String mongodb = args[1];
                        String refcollection = Context.getReferencesCollection();
                        String linkcollection = Context.getCitationsCollection();
                        ...
                        if (args.length == 4) {
                            if (args[3].equals("true")) {
                                ...
                                consolidationDatabase = args[3];
                            }
                        }
                        CitationNetwork.fillCitationsMongoIds(mongodb, refcollection, linkcollection, idsfile, consolidationDatabase);
                    }
                }
